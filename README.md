...
```bash
# 1. 拉取本项目
git clone https://github.com/YangLv2023/homy-ha-bridge.git
cd homy-ha-bridge

# 2. 准备外部网络（HA 容器需与 broker 同网络，按需改名）
...
# 4. 启动（mosquitto + bridge 一起拉起）
docker compose up -d --build
```

启动后查看 `docker logs homy-bridge`：登录成功、家庭解析完成、retained 消息发布完成后，HA 里即可配置 MQTT 集成。

### HA 侧配置（首次一次性）

1. HA 左侧栏：**设置 → 设备与服务 → 添加集成**
2. 搜索并选择 **MQTT**
3. 填写连接信息：
   - **broker**：mosquitto 所在主机的地址（桥接与 HA 同 Docker 网络时填容器名 `homy-mosquitto`；HA 跑在别的机器时填该主机 IP）
   - **端口**：`1883`
   - **用户名 / 密码**：与 `.env` 里的 `MQTT_USER` / `MQTT_PASS` 一致
4. 保存后无需其他操作，桥接发布的 Discovery 消息会自动生成设备与实体（MQTT 集成默认开启「发现」）
...
APP 设备名 → HA 实体名的翻译表，**全文件不填设备 ID**，启动时按名字自动解析。

- 仓库里默认带了一份可直接运行的配置，开箱即用
- 自定义时只需填三类信息：
  1. 每台设备的 `name`（HA 里显示的名字）和 `room`（归到哪个区域）
  2. 智控面板在 `channels` 里按 `switch_1..4` 逐键起名
  3. 名字写法：`设备名`（全家唯一）/ `房间/设备名`（区分房间）/ `房间/设备名@N`（同房间同名多台，N=第几台）

整个文件删掉也能跑：HA 用 APP 原名，暖通照常出实体，只有面板出不了灯。名字对不上时启动会报错并列出 APP 现有名字，照着改即可。
...
- **禁止商业使用**：未经授权，不得将本项目用于任何商业目的
- **禁止非法用途**：不得利用本项目从事任何违反法律法规的活动
- **遵守服务条款**：使用本项目时请遵守相关平台的服务条款和使用协议
- **风险自负**：使用本项目所产生的一切后果由使用者自行承担，与项目作者无关
- **尊重知识产权**：请尊重原作者及相关平台的知识产权
